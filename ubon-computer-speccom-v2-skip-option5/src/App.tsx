 ...
 * - Required categories (configurable) + warnings when skipped
 * - Pricing: VAT, Discount, Cost, Profit
 * - Attribute filters (socket, ramType, formFactor, storage interface, PSU wattage, cooler socket)
 * - All previous V.2 features (search, sort, add-ons, import, reset)
 */
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-end"><Button onClick={()=>{
        try{ const attrs = JSON.parse(attributes || "{}"); onSave({ id:(initial?.id as string)||uid(), name, category, price, stock, cost, attributes: attrs } as Product); }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="rounded-2xl p-5 mb-3 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 text-white shadow">
        <motion.h1 initial={{opacity:0, y:-8}} animate={{opacity:1, y:0}} className="text-2xl md:text-3xl font-bold tracking-tight">
          Ubon Computer SpecCom V.3 — จำเป็นต้องเลือกหมวด • ภาษี/ส่วนลด/กำไร • ตัวกรอง Attribute
        </motion.h1>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <Select value={filters.socket || ""} onValueChange={(v)=>setFilters({...filters, socket: v || undefined})}>
                    <SelectTrigger><SelectValue placeholder="ทั้งหมด" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">ทั้งหมด</SelectItem>
                      {socketOptions.map(s=>(<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label>RAM Type (MB/RAM)</Label>
                  <Select value={filters.ramType || ""} onValueChange={(v)=>setFilters({...filters, ramType: v || undefined})}>
                    <SelectTrigger><SelectValue placeholder="ทั้งหมด" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">ทั้งหมด</SelectItem>
                      {ramTypeOptions.map(s=>(<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label>Form Factor (MB/Case)</Label>
                  <Select value={filters.formFactor || ""} onValueChange={(v)=>setFilters({...filters, formFactor: v || undefined})}>
                    <SelectTrigger><SelectValue placeholder="ทั้งหมด" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">ทั้งหมด</SelectItem>
                      {formFactorOptions.map(s=>(<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label>Storage Interface (MB/Storage)</Label>
                  <Select value={filters.storageInterface || ""} onValueChange={(v)=>setFilters({...filters, storageInterface: v || undefined})}>
                    <SelectTrigger><SelectValue placeholder="ทั้งหมด" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">ทั้งหมด</SelectItem>
                      {storageIfOptions.map(s=>(<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label>PSU กำลังไฟขั้นต่ำ</Label>
                  <Input type="number" value={typeof filters.minPSUWatt==="number"?filters.minPSUWatt:""} onChange={e=>setFilters({...filters, minPSUWatt: e.target.value? Number(e.target.value): undefined})} placeholder="เช่น 650" />
                </div>
                <div className="space-y-1">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 border-t pt-4 space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground">คอลัมน์ที่รองรับ: Category, Name, Price, Cost, Stock, Attributes ... (รองรับ Monitor/Software)</div>
                </div>
              </CardContent>
            </Card>
          </div>
