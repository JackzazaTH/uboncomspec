...
import {
  Plus, Trash2, Edit, HardDrive, Cpu,
  Power, Monitor as MonitorIcon, Wrench,
  Download, FileUp, Save, ListChecks, ShoppingCart, CheckCircle2,
  AlertTriangle, Sparkles, X, Package, Box
} from "lucide-react";
import * as XLSX from "xlsx";

/**
 * Ubon Computer Spec V.4 — Single-file UI-focused version (finalized)
 * - Cleaner header / layout, clearer controls, gentle animations
...
type BuildState = { base: Partial<Record<BaseCategory, Product>>; addons: AddonEntry[] };
...
const STORAGE_KEYS = {
  inventory: "ubonspec.inventory.v4",
  build: "ubonspec.build.v4",
  pricing: "ubonspec.pricing.v4",
  required: "ubonspec.required.v4",
  filters: "ubonspec.filters.v4",
  smartSync: "ubonspec.smartsync.v4",
} as const;
...
const categoryIcon: Record<Category, React.ReactNode> = {
  ...
  GPU: <MonitorIcon className="w-4 h-4" />,
  RAM: <Package className="w-4 h-4" />,
  Storage: <HardDrive className="w-4 h-4" />,
  PSU: <Power className="w-4 h-4" />,
  Case: <Box className="w-4 h-4" />,
  Cooler: <Wrench className="w-4 h-4" />,
  Monitor: <MonitorIcon className="w-4 h-4" />,
...
function ProductEditor({ initial, onSave, onClose }: { initial?: Partial<Product>, onSave: (p: Product) => void, onClose: ()=>void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-between items-center">
        <Button variant="secondary" onClick={onClose}><X className="w-4 h-4 mr-2"/> ยกเลิก</Button>
        <Button onClick={()=>{
        ...
        }}><Save className="w-4 h-4 mr-2"/> บันทึกสินค้า</Button>
      </div>
    </div>
  );
}
...
function BasePicker({ inventory, selection, onSelect, sortMode, onChangeSort, required, filters }:{ inventory: Product[]; selection: Partial<Record<BaseCategory, Product>>; onSelect:(cat:BaseCategory, product:Product|null)=>void; sortMode: SortMode; onChangeSort:(m:SortMode)=>void; required: BaseCategory[]; filters: AttrFilters; }){
  ...
  const [showAllComp, setShowAllComp] = React.useState(false);
  ...
  return (
    <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="col-span-8 flex gap-2">
                <Select value={selection[cat]?.id || "__none__"} onValueChange={(id)=>{ if(id==="__none__"){ onSelect(cat, null); if (required.includes(cat)) toast.error(`${cat} เป็นหมวดจำเป็น`); return; } const item = byCat[cat].find(p=>p.id===id) || null; onSelect(cat, item); }}>
                  ...
                  <SelectContent>
                    ...
                    {sortProducts(byCat[cat].filter(p => !searchLower || p.name.toLowerCase().includes(searchLower)), sortMode).map(p => (
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          <div className="space-y-2">
            {(showAllComp ? comp.notes : comp.notes.filter(n=>n.level!=="ok").slice(0,4)).map((n,i)=>(
              <div key={i} className={`flex items-start gap-2 rounded-xl px-3 py-2 ${n.level==='error'?'bg-red-50 text-red-700':n.level==='warn'?'bg-amber-50 text-amber-700':'bg-emerald-50 text-emerald-700'}`}>
                {n.level!=='ok'?<AlertTriangle className="w-4 h-4 mt-0.5"/>:<CheckCircle2 className="w-4 h-4 mt-0.5"/>}
...
function Summary({ build, onReset, pricing, setPricing, required }:{ build: BuildState, onReset: ()=>void, pricing: Pricing, setPricing: (p:Pricing)=>void, required: BaseCategory[] }){
  ...
  const baseSelected = baseAllRows.filter(r=>!!r.p) as {c: BaseCategory, p: Product}[];
...
function deriveFiltersFromSelection(base: Partial<Record<BaseCategory, Product>>, current: AttrFilters): AttrFilters {
  ...
  if (!storage?.attributes?.interface && Array.isArray(mb?.attributes?.storage) && (mb!.attributes.storage as string[]).length>0) next.storageInterface = (mb!.attributes.storage as string[])[0];
...
export default function App(){
  ...
  useEffect(() => {
    document.title = "Ubon Computer Spec V.4";
  }, []);

  const fileRef = useRef<HTMLInputElement>(null);

  const filtered = useMemo(()=> inventory, [inventory]);
  ...
  useEffect(()=>{
    if (!smartSync) return;
    const next = deriveFiltersFromSelection(build.base, {});
    setFilters(next);
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {BASE_CATEGORIES.map(c=>{
                  const active = (required as BaseCategory[]).includes(c);
                  return <Button key={c} variant="secondary" onClick={()=>{
                    const next = active ? required.filter(x=>x!==c) : [...required, c];
                    setRequired(next as BaseCategory[]);
                    toast.message(active ? `ยกเลิกหมวดจำเป็น: ${c}` : `กำหนดหมวดจำเป็น: ${c}`);
                  }}>{active ? `✓ ${c}` : c}</Button>;
                })}
              </div>
              <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm font-medium">ตัวกรอง Attribute</div>
                <div className="flex gap-2">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  <Label>Storage I/F (MB/Storage)</Label>
                  <div className="flex flex-wrap gap-2">
        ...
        <TabsContent value="inventory" className="space-y-4">
          <Card className="shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <input ref={fileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={(e)=>{ const f=e.target.files?.[0]; if(f) handleImport(f); (e.target as any).value=null; }} />
                <Button variant="secondary" onClick={()=>fileRef.current?.click()}><FileUp className="w-4 h-4 mr-2"/> นำเข้า Excel</Button>
                <Button onClick={()=>setEditing({} as any)}><Plus className="w-4 h-4 mr-2"/> เพิ่มสินค้า</Button>
              </div>
            </CardHeader>
            <CardContent>
              <InventoryTable
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between w-full">
              ...
              <button className="rounded-full p-1 hover:bg-muted" onClick={()=>setEditing(null)} aria-label="ปิด">
                <X className="w-4 h-4"/>
              </button>
            </DialogTitle>
          </DialogHeader>
          <ProductEditor initial={editing || undefined} onSave={saveProduct} onClose={()=>setEditing(null)} />
          <DialogFooter />
        </DialogContent>
      </Dialog>
    </div>
