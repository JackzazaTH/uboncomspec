...
// ===== Base Picker =====
function BasePicker({ inventory, selection, onSelect }:{ inventory: Product[]; selection: Partial<Record<BaseCategory, Product>>; onSelect:(cat:BaseCategory, product:Product|null)=>void; }){
  const byCat = useMemo(()=>{
...
// ===== Add-ons Picker =====
function AddonsPicker({ inventory, addons, addAddon, updateQty, removeAddon }:{ inventory: Product[]; addons: AddonEntry[]; addAddon:(p:Product, qty:number)=>void; updateQty:(id:string, qty:number)=>void; removeAddon:(id:string)=>void; }){
  const byCat = useMemo(()=>{
        ...
        <div className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
          ...
          <div className="col-span-6">
            <Select value={selMonitor} onValueChange={setSelMonitor}>
              ...
              <SelectContent>
                {byCat["Monitor"].map(p=>(<SelectItem key={p.id} value={p.id}>{p.name} — {baht(p.price)}</SelectItem>))}
        ...
        <div className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
          ...
          <div className="col-span-6">
            <Select value={selSoftware} onValueChange={setSelSoftware}>
              ...
              <SelectContent>
                {byCat["Software"].map(p=>(<SelectItem key={p.id} value={p.id}>{p.name} — {baht(p.price)}</SelectItem>))}
...
export default function App(){
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      {/* Modern header */}
      <div className="rounded-2xl p-5 mb-5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 text-white shadow">
      ...
      <Tabs defaultValue="builder" className="space-y-4">
        ...
        <TabsContent value="builder" className="space-y-4">
          <BasePicker inventory={inventory} selection={build.base} onSelect={selectBase} />
          <AddonsPicker inventory={inventory} addons={build.addons} addAddon={addAddon} updateQty={updateAddonQty} removeAddon={removeAddon} />
          {/* Sticky total */}
